function [all_points_set, suture_tip1]  = func_intersected_suture_thread_detection_with_clicks(image1 , image1_origin)

    %image1_origin = imread('C:\Users\User\OneDrive - The Hong Kong Polytechnic University\PhD Studies\Semester 8\suture_with_needle\test_set\2.jpg');
    %image1 = rgb2gray(image1_origin);
    ...
    %tic;
    outIm1 = FrangiFilter2D(double(image1));
    ...
    er_kernal = ones(expansion_threshold, expansion_threshold, 'gpuArray');
    biOutIm1 = imerode(Original_biOutIm1, er_kernal);
    
    biOutIm1 = biOutIm1 * (-1) + 1;
    ...
    for angle_variation_group = 1 : 1
        for P1_variation = 1 : 1
            for P2_variation = 1 : 1    
                ...
                thre_out_of_zone = round(RADIUS / 5);
                ...
                %
                scatter(CURRENT_POINT(1, 1), CURRENT_POINT(1, 2), 20 , 'r');
                hold on; pause(0.03);
                %
                i = i + 1; 
                ...
                if (size(POINT_SEQUENCE, 1) > FINAL_ELEMENT_NUM)
                    ...
                    POINT_SEQUENCE_OPTIMIZE = [];
                    POINT_SEQUENCE_OPTIMIZE = POINT_SEQUENCE;
                end
    % -----------------------------------------------------------------             
            end
        end
    ...
    POINT_SEQUENCE_OPTIMIZE = [suture_tip1 ; POINT_SEQUENCE];
    %%
    % point_set_optimizer
    angle_opti_threshold = 5;

    for point_num = 2 : size(POINT_SEQUENCE_OPTIMIZE, 1) - 1

        point_last    = POINT_SEQUENCE_OPTIMIZE(point_num - 1, :);
        point_current = POINT_SEQUENCE_OPTIMIZE(point_num    , :);
        point_next    = POINT_SEQUENCE_OPTIMIZE(point_num + 1, :);
    ...
    %{
    % because of the noisy environment, we can ignore the optimization of
    % the end point
    ...
    POINT_SEQUENCE_FINAL = round([POINT_SEQUENCE_FINAL; POINT_SEQUENCE_OPTIMIZE(end, :)]);
