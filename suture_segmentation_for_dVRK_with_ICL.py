# -*- coding: utf-8 -*-
"""
Created on Tue Feb 18 15:54:54 2020
...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 16 17:52:19 2019
...
def DL_based_segmentation(frame, erode_kernel, dilate_kernel):
...
def change_contrast_brightness(image):
...
#########################################################################
#########################################################################
data_path = 'C:/Users/User/OneDrive - The Hong Kong Polytechnic University/Postdoc_works/2019-second_half/Research_works/Suturing/Paper_preparation/Experimental figures/suture_grasping_dVRK_ICL/time_3/Exp_3/'
compute_path = 'C:/Users/User/OneDrive - The Hong Kong Polytechnic University/PhD Studies/Semester 4/3D Reconstruction/2017.03.20/v-repApi/'

#original_image = cv2.imread(data_path, 'stereo_image')
#O_L = np.shape(original_image)[1]
#O_H = np.shape(original_image)[0]

#cv2.imwrite((compute_path + 'left_image.jpg'), original_image[:, 0, int(O_L / 2)]  )
#cv2.imwrite((compute_path + 'right_image.jpg'), original_image[:, int(O_L / 2) , O_L]  )

frame_left = cv2.imread((data_path + 'left_image.jpg'))
frame_right = cv2.imread((data_path + 'right_image.jpg'))
cv2.imwrite((compute_path + 'left_image.jpg'), frame_left  )
cv2.imwrite((compute_path + 'right_image.jpg'), frame_right  )
...
model.load_weights(model_path + 'unet_gray_ICL_dVRK_1_40_direct_trian.h5')
#model.load_weights(model_path + 'unet_color_image_1to40_fine_tune.h5')
model.load_weights(model_path + 'unet_gray_ICL_dVRK_1_to_40_fine_tune.h5')
...
while(True):
    ...
    #ret, frame = cap.read()
    frame_left = cv2.imread(compute_path + 'left_image.jpg')
    frame_right = cv2.imread(compute_path + 'right_image.jpg')
    
    #frame_left = change_contrast_brightness(frame_left)
    #frame_right = change_contrast_brightness(frame_right)
    
    img_length = np.shape(frame_left)[1]
    img_height = np.shape(frame_left)[0]
    
    if (np.size(frame_left) > 10) & (np.size(frame_right) > 10):
        
        #frame = cv2.flip(frame,0)
        segmented_results_l = DL_based_segmentation(frame_left, erode_kernel, dilate_kernel)
        segmented_results_r = DL_based_segmentation(frame_right, erode_kernel, dilate_kernel)
        
        segmented_results_sizeBack_l = cv2.resize(segmented_results_l, (img_length, img_height))
        segmented_results_sizeBack_r = cv2.resize(segmented_results_r, (img_length, img_height))
        
        cv2.imwrite((compute_path + 'segmented_suture_l.jpg'), segmented_results_sizeBack_l)
        cv2.imwrite((compute_path + 'segmented_suture_r.jpg'), segmented_results_sizeBack_r)
              
        numpy_vertical_concat = np.concatenate((segmented_results_sizeBack_l, segmented_results_sizeBack_r), axis=1 )
