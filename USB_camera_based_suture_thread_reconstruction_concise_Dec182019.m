...
while iteration_steps <= 1    
    ...
    %a_t_1 = all_points_set_1(end: -1 : 1, :); a_t_2 = all_points_set_2(end: -1 : 1, :);
    %all_points_set_1 = a_t_1; all_points_set_2 = a_t_2; 
...
tem_value = [Hcam2marker_ * [multi_view_coordinates_RCM_vision{1, :}, repmat(1, size(multi_view_coordinates_RCM_vision{1, :}, 1), 1)]']';
tem_value(1, 1 : 3)
%multi_view_coordinates_RCM_vision{1, 1} = tem_value(:, 1 : 3);
%multi_view_coordinates_RCM_vision{1, 1}(1, :)
...
for i = 1 : Cell_size
    ...
    for j = 2 : size(multi_view_coordinates_RCM_vision{i}, 1)
        plot3([Input_xyz(j - 1, 1), Input_xyz(j, 1)], ...
              ...
              'Color', dc(c_size, :), 'LineWidth', 5); hold on;
         c_size = c_size + 1; 
    end
...
[t11, t22, CoCo] = dVRK_get_transformation_and_coordinates('J1_ECM', 'J1_PSM1', Optimized_point_set);
