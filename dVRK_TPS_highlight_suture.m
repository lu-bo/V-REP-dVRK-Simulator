function [Tx_l, Ty_l, Tx, Ty] = dVRK_TPS_highlight_suture(I0_color, I1_color, Range1, Dist_XY, left_suture, right_suture)
%     I0_color = left_image;
%     I1_color = rotated_image;
%     Range1 = Range;
%     Dist_XY = Dist_XY;
    ...
    Template_suture_left = double(left_suture(T_Y_STAR:T_Y_STAR+T_Y_SIZE-1, T_X_STAR:T_X_STAR+T_X_SIZE-1));
    ...
    %     [I0, I1] = rectifyStereoImages(I0,I1,stereoParams);
        ...
        for t = 1:max_iteration
            ...
            warped_added = interp2(right_suture, Tx, Ty, 'Bilinear');
            
            warped(isnan(warped)) = 0;
            warped_added(isnan(warped_added)) = 0;
            % jacobian
            [Gx_1, Gy_2] = imgradientxy(warped);
            [Gx_added, Gy_added] = imgradientxy(warped_added);
            Gx = Gx_1 + 1/(log(10)^2)*Gx_added; 
            Gy = Gy_2 + 1/(log(10)^2)*Gy_added; 
            ...
            % dI
            It = warped(:) - Template(:);
            It_added = 0.2*(abs(warped_added(:) - Template_suture_left(:))); % .* (warped_added(:) - Template_suture_left(:));
            
            
            du = -2 * pinv(W) * (It + It_added);
            dx = du(1:c_size);
        ...
        error(k) = sum(abs(It))+ sum(abs(It_added));
        figure(2);
        imshow(uint8(warped));
