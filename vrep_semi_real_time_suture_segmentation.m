...
X_Size = round(1.4*x_diff); Y_Size = round(1.4*y_diff);
X_Start = 0.5 * (x_min + x_max - X_Size); Y_Start = 0.5 * (y_min + y_max - Y_Size);

Range = [X_Start, Y_Start, X_Size, Y_Size];
Dist_XY = [round(X_Size/4); round(Y_Size/4)];

[shape_points, Point_color] = dVRK_TPS(left_image, right_image, Range, Dist_XY, stereoParams);
...
zlabel('Z/mm', 'fontsize', 25, 'fontweight', 'bold');
hold on;
plot3(1000*xyzsg(:,1), 1000*xyzsg(:,2), 1000*xyzsg(:,3), 'r');
grid on;
